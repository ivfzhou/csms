...
var (
	...
	APIAccount = &model.APIAccount{
		...
		Secret:      util.FastRandomAlphaNumberString(32),
		IP:          model.StringList{"*"},
		Frequency:   100,
		ExpiredTime: time.Now().Add(24 * time.Hour),
	}
	AppleCertificateCertDERBase64 string
	RedisShakeScriptSha           = util.FastRandomAlphaNumberString(32)
	RedisRateLimitScriptSha       = util.FastRandomAlphaNumberString(32)
)
...
func init() {
	...
	mvt.Chain(cfg.Get()).Elem().FieldByName("EnvironmentValue").Set(cfg.EnvironmentProduction)
	mvt.Chain(cfg.Get()).Elem().FieldByName("LogConfiguration").FieldByName("LevelValue").Set(log.LevelFatal.String())
	privateKeyPEM, _, _ := GenerateECDSAKeyPEM("P256")
	mvt.Chain(cfg.Get()).Elem().FieldByName("AppleAPIConfiguration").FieldByName("IssuerIDValue").Set("test-issuer-id")
	mvt.Chain(cfg.Get()).Elem().FieldByName("AppleAPIConfiguration").FieldByName("KeyIDValue").Set("test-key-id")
	mvt.Chain(cfg.Get()).Elem().FieldByName("AppleAPIConfiguration").FieldByName("SecretValue").Set(privateKeyPEM)
	...
	mvt.Chain(cfg.Get()).Elem().FieldByName("AppleConfiguration").FieldByName("CertificatePrivateKeyValue").Set(keyPEM)
}
...
func CreateAppleAPIResponse(body string) *http.Response {
...
func CreateAPIPostMultiFormRequest(ctx context.Context, uri string, req io.Reader, contentType string) *http.Request {
	request := httptest.NewRequest(http.MethodPost, uri, req)
	request.Header.Set("Date", time.Now().UTC().Format("Mon, 02 Jan 2006 15:04:05 GMT"))
	request.Header.Set("Content-Type", contentType)
	request.Header.Set("X-Real-IP", RequestIP)
	request.Header.Set("Authorization", CreateAPIAuthorization(AppInfo.AppID, APIAccount.AccountID, APIAccount.Secret))
	return request.WithContext(ctx)
}
