...
// FileWebDownloadReq 下载请求体。
type FileWebDownloadReq struct {
	// 文件 ID
	FileID string `form:"fileId" binding:"alphanum,len=38" example:"2026014ef83c03e2ce4f1f94c11168d1acd087"`
}

// FileWebInitialReq 初始化上传请求体。
type FileWebInitialReq struct {
	// 应用 ID
...
// FileWebInitialRsp 初始化上传响应体。
type FileWebInitialRsp struct {
	// 文件是否存在
...
// FileWebUploadPartReq 上传分片请求体。
type FileWebUploadPartReq struct {
	// 文件分片
	Chunk *multipart.FileHeader `form:"chunk" binding:"file"`
	// 文件 ID
	FileID string `form:"fileId" binding:"alphanum,len=38"`
	// 分片序号
	ChunkNumber int `form:"chunkNumber" binding:"gt=0"`
}

// FileWebMergePartsReq 合并分片请求体。
type FileWebMergePartsReq struct {
	// 文件 ID
	FileID string `form:"fileId" binding:"alphanum,len=38" example:"2026014ef83c03e2ce4f1f94c11168d1acd087"`
}

func initFileWebProtocol() {
	validator.AddTranslationMessage(reflect.TypeFor[FileWebDownloadReq](), validator.TranslationMessage{
		"FileID": {
			"alphanum": {
	...
	validator.AddTranslationMessage(reflect.TypeFor[FileWebInitialReq](), validator.TranslationMessage{
		"AppID": {
			"len": {
	...
	validator.AddTranslationMessage(reflect.TypeFor[FileWebUploadPartReq](), validator.TranslationMessage{
		"Chunk": {
			"file": {
	...
	validator.AddTranslationMessage(reflect.TypeFor[FileWebMergePartsReq](), validator.TranslationMessage{
		"FileID": {
			"alphanum": {
				i18n.LanguageChinese: "不存在待上传文件",
				i18n.LanguageEnglish: "there is no file to be uploaded",
