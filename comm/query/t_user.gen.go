...
func newUser(db *gorm.DB, opts ...gen.DOOption) user {
	...
	_user.NameEn = field.NewString(tableName, "name_en")
	_user.NameZh = field.NewString(tableName, "name_zh")
	_user.AvatarFileID = field.NewString(tableName, "avatar_file_id")
	...
	_user.CreatedTime = field.NewTime(tableName, "created_time")
...
type user struct {
	...
	NameZh         field.String // 中文名
	AvatarFileID   field.String // 头像文件 ID
	Department     field.String // 部门组织，以/分隔
	PasswordDigest field.String // 密码 MD5
	PasswordSalt   field.String // 密码盐值
...
func (u *user) updateTableName(table string) *user {
	...
	u.NameZh = field.NewString(table, "name_zh")
	u.AvatarFileID = field.NewString(table, "avatar_file_id")
...
func (u *user) fillFieldMap() {
	...
	u.fieldMap["name_zh"] = u.NameZh
	u.fieldMap["avatar_file_id"] = u.AvatarFileID
...
type IUserDo interface {
	...
	SearchByName(name string) (result []*model.User, err error)
}
...
// limit 10
func (u userDo) SearchByName(name string) (result []*model.User, err error) {
	...
	params = append(params, name)
	params = append(params, name)
	generateSQL.WriteString("select name_en, name_zh from t_user where name_en like concat('%', concat(?, '%')) or name_zh like concat('%', concat(?, '%')) order by case when name_en = ? or name_zh = ? then 1 else 2 end, id desc limit 10 ")
