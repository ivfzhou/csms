...
func newFile(db *gorm.DB, opts ...gen.DOOption) file {
	...
	_file.ID = field.NewInt(tableName, "id")
	_file.FileID = field.NewString(tableName, "file_id")
	_file.TusdID = field.NewString(tableName, "tusd_id")
	_file.UserID = field.NewInt(tableName, "user_id")
	_file.APIAccountID = field.NewInt(tableName, "api_account_id")
	_file.AppID = field.NewInt(tableName, "app_id")
	_file.Name = field.NewString(tableName, "name")
	_file.Md5 = field.NewString(tableName, "md5")
	_file.Size = field.NewInt(tableName, "size")
	_file.Type = field.NewInt(tableName, "type")
	_file.CreatedTime = field.NewTime(tableName, "created_time")
...
type file struct {
	...
	ALL          field.Asterisk
	ID           field.Int    // 自增主码
	FileID       field.String // 文件 ID，前 6 位是年月，后 32 位是 UUID
	TusdID       field.String // 在 Tusd 服务器的 ID
	UserID       field.Int    // 上传人 ID
	APIAccountID field.Int    // 上传凭证 ID
	AppID        field.Int    // 所属应用 ID
	Name         field.String // 文件名
	Md5          field.String // 文件摘要
	Size         field.Int    // 文件大小
	Type         field.Int    // 类型；1=用户头像、2=应用图标、3=Android 签名文件、4=Windows 签名文件、5=Apple 签名文件、6=HLK 日志文件、7=微软方的结果文件
	CreatedTime  field.Time   // 上传时间
...
func (f *file) updateTableName(table string) *file {
	...
	f.TusdID = field.NewString(table, "tusd_id")
	f.UserID = field.NewInt(table, "user_id")
	f.APIAccountID = field.NewInt(table, "api_account_id")
	f.AppID = field.NewInt(table, "app_id")
...
func (f *file) fillFieldMap() {
	...
	f.fieldMap["tusd_id"] = f.TusdID
	f.fieldMap["user_id"] = f.UserID
	f.fieldMap["api_account_id"] = f.APIAccountID
